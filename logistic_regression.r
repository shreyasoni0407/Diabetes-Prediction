...
#------------logistic_Boruta_Undersample---------------

logistic_model <- glm(Class ~ ., data = Boruta_Undersampled, family = "binomial")
summary(logistic_model_over)
logistic_model_over.pred <- predict(logistic_model_over, ts, type = "response")
pred <- factor(ifelse(logistic_model_over.pred >= 0.5, 1, 0))
cm <- table(pred, ts$Class)
cm
accuracy <- round((sum(diag(cm)) / sum(cm)) * 100, 3)
cat("Accuracy: ", accuracy, "%\n")
predicted_probabilities <- predict(logistic_model, newdata = ts, type = "response")
roc_score=roc.curve(ts$Class, predicted_probabilities, levels = levels(ts$Class))
roc_score

#-------------logistic_Boruta_Oversample---------------
logistic_model <- glm(Class ~ ., data = Boruta_Oversampled, family = "binomial")
summary(logistic_model_over)
logistic_model_over.pred <- predict(logistic_model_over, ts, type = "response")
pred <- factor(ifelse(logistic_model_over.pred >= 0.5, 1, 0))
cm <- table(pred, ts$Class)
...
#-------------logistic on elasticnet oversampled-----------------------
logistic_model <- glm(Class ~ ., data = ElasticNet_Oversample, family = "binomial")
summary(logistic_model_Elasticnet_over)
logistic_model_Elasticnet_over.pred <- predict(logistic_model_Elasticnet_over, ts, type = "response")
pred <- factor(ifelse(logistic_model_Elasticnet_over.pred >= 0.5, 1, 0))
cm <- table(pred, ts$Class)
cm
accuracy <- round((sum(diag(cm)) / sum(cm)) * 100, 3)
cat("Accuracy: ", accuracy, "%\n")
...
roc_score=roc.curve(ts$Class, predicted_probabilities, levels = levels(ts$Class))
roc_score
#--------------logistic on elasticnet undersampled---------------------
logistic_model_Elast <- glm(Class ~ ., data = ElasticNet_UnderSample, family = "binomial")
summary(logistic_model_Elasticnet_under)
logistic_model_Elasticnet_under.pred <- predict(logistic_model_Elasticnet_under, ts, type = "response")
pred <- factor(ifelse(logistic_model_Elasticnet_under.pred >= 0.5, 1, 0))
cm <- table(pred, ts$Class)
cm
accuracy <- round((sum(diag(cm)) / sum(cm)) * 100, 3)
cat("Accuracy: ", accuracy, "%\n")
...
#----------------logistic on lasso under----------------------------
logistic_model <- glm(Class ~ ., data = Lasso_Oversample, family = "binomial")
summary(logistic_model_lasso_over)
logistic_model_lasso_over.pred <- predict(logistic_model_lasso_over, ts, type = "response")
pred <- factor(ifelse(logistic_model_lasso_over.pred >= 0.5, 1, 0))
cm <- table(pred, ts$Class)
cm
accuracy <- round((sum(diag(cm)) / sum(cm)) * 100, 3)
cat("Accuracy: ", accuracy, "%\n")
...
logistic_model <- glm(Class ~ ., data = Lasso_Undersample, family = "binomial")
summary(logistic_model_lasso_under)
logistic_model_lasso_over.pred <- predict(logistic_model_lasso_under, ts, type = "response")
pred <- factor(ifelse(logistic_model_lasso_over.pred >= 0.5, 1, 0))
cm <- table(pred, ts$Class)
cm
accuracy <- round((sum(diag(cm)) / sum(cm)) * 100, 3)
