...
# --------random forest for boruta oversample -----------------------

rf_model <- randomForest(Class ~ ., data = Boruta_Oversample, ntree = 200, mtry = 5)
predicted <- predict(rf_model, ts)
cm <- table(predicted, ts$Class)
print(cm)
accuracy <- sum(diag(cm)) / sum(cm)
cat("Overall Accuracy: ", round(accuracy * 100, 3), "%\n")
predicted_probabilities <- predict(rf_model, newdata = ts, type = "response")
roc_score=roc.curve(ts$Class, predicted_probabilities, levels = levels(ts$Class))
roc_score

#-----------random forest for boruta undersample----------------

rf_model <- randomForest(Class ~ ., data = Boruta_Undersample, ntree = 200, mtry = 3)
predicted <- predict(rf_model, ts)
cm <- table(predicted, ts$Class)
print(cm)
accuracy <- sum(diag(cm)) / sum(cm)
...
#-----------------random forest for elasticnet over ------------------

rf_model <- randomForest(Class ~ ., data = ElasticNet_Oversample, ntree = 200, mtry = 5)
predicted <- predict(rf_model, ts)
cm <- table(predicted, ts$Class)
print(cm)
accuracy <- sum(diag(cm)) / sum(cm)
...
#--------------------randomm forest for elasticnet under----------------

rf_model <- randomForest(Class ~ ., data = ElasticNet_UnderSample , ntree = 300, mtry = 3)
predicted <- predict(rf_model, ts)
cm <- table(predicted, ts$Class)
print(cm)
accuracy <- sum(diag(cm)) / sum(cm)
...
#------------------------------------------ rf for lasso over-------------------------
rf_model <- randomForest(Class ~ ., data =Lasso_Oversample, ntree = 300, mtry = 3)
predicted <- predict(rf_model, ts)
cm <- table(predicted, ts$Class)
print(cm)
accuracy <- sum(diag(cm)) / sum(cm)
...
#-------------------------------rf for lasso under-------------------------------------
rf_model <- randomForest(Class ~ ., data =Lasso_Undersample, ntree = 300, mtry = 3)
predicted <- predict(rf_model, ts)
cm <- table(predicted, ts$Class)
print(cm)
accuracy <- sum(diag(cm)) / sum(cm)
